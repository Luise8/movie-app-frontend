...
const genresToTestId = genresTMDB.slice(0, 2).map((item) => item.id).join(',');
const genresToTestNames = genresTMDB.slice(0, 2).map((item) => item.name).join(', ');

const mockData = testDbHelpers.moviesByGenre;
// mock get data service
jest.mock('src/services/get-data', () => ({
  getMoviesByGenre: jest.fn(() => (Promise.resolve(mockData))),
}));
...
it('render right of initial page data after loading state', async () => {
  await act(async () => {
    render(
      <MemoryRouter initialEntries={[`/genre/${genresToTestId}`]}>
  ...
  expect(getMoviesByGenre).toBeCalled();
  ...
  const page = within(pageLayout).getByTestId('page-genre');
  expect(page).toBeInTheDocument();

  const h1Matcher = 'Movies by genre';
  ...
  expect(screen.getByText(new RegExp(genresMatcher, 'i'))).toBeInTheDocument();
...
it('Render the correct error page when there is any error other than 404', async () => {
  getMoviesByGenre.mockImplementationOnce(() => Promise.reject(new Error('something wrong')));
  ...
  expect(screen.queryByTestId(/page-genre/i)).not.toBeInTheDocument();
...
// Change devolution of get data service
  ...
  expect(screen.getByTestId('page-genre')).toHaveClass('page');
