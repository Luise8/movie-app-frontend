...
import { getRatedMovies } from 'src/services/get-data';

const mockData = testDbHelpers.moviesRated;
// mock get data service
jest.mock('src/services/get-data', () => ({
  getRatedMovies: jest.fn(() => (Promise.resolve(mockData))),
}));
...
it('render right of initial page data after loading state', async () => {
  ...
  expect(getRatedMovies).toBeCalled();
  ...
  const page = within(pageLayout).getByTestId('page-rated');
  expect(page).toBeInTheDocument();

  const h1Matcher = 'rated movies';
...
it('Render the correct error page when there is any error other than 404', async () => {
  getRatedMovies.mockImplementationOnce(() => Promise.reject(new Error('something wrong')));
  ...
  expect(screen.queryByTestId(/page-rated/i)).not.toBeInTheDocument();
...
// Change devolution of get data service
  ...
  expect(screen.getByTestId('page-rated')).toHaveClass('page');
