...
import { getLatestMovies } from 'src/services/get-data';

const mockData = testDbHelpers.moviesLatest;
// mock get data service
jest.mock('src/services/get-data', () => ({
  getLatestMovies: jest.fn(() => (Promise.resolve(mockData))),
}));
...
it('render right of initial page data after loading state', async () => {
  ...
  expect(getLatestMovies).toBeCalled();
  ...
  const page = within(pageLayout).getByTestId('page-latest');
  expect(page).toBeInTheDocument();

  const h1Matcher = 'latest movies';
...
it('Render the correct error page when there is any error other than 404', async () => {
  getLatestMovies.mockImplementationOnce(() => Promise.reject(new Error('something wrong')));
  ...
  expect(screen.queryByTestId(/page-latest/i)).not.toBeInTheDocument();
...
// Change devolution of get data service
  ...
  expect(screen.getByTestId('page-latest')).toHaveClass('page');
