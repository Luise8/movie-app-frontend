...
export default function useOneListUser({ userId, listId }) {
  const INITIALPAGE = 0;
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const dataFetched = await getOneListUser({ page: INITIALPAGE, userId, listId });
        if (mounted) {
          setData(dataFetched);
          setError(false);
          setLoading(false);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const dataFetched = await getOneListUser({ page, userId, listId });
        if (mounted) {
          setData((prevData) => {
